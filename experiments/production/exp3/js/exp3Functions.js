...
function update_deck_attributes(deckCode) {
  ...
  for (var horizontal = 0; horizontal < left_right.length; horizontal++) {
    ...
    probability_string = deckCode[left_right[horizontal]]['probability'][top_bottom[vertical]].toString();
    full_string = outcome_string + ' (' + probability_string + ')';
    ...
    $('#' + left_right[horizontal] + 'Deck' + jsUcfirst(top_bottom[vertical])).html('<p>' + full_string + '</p>');
...
function update_attribute_colors(deckCode) {
  ...
  for (var horizontal = 0; horizontal < left_right.length; horizontal++) {
    for (var vertical = 0; vertical < top_bottom.length; vertical++) {
      ...
      if (outcome > 0) {
        $('#' + left_right[horizontal] + 'Deck' + jsUcfirst(top_bottom[vertical])).css({'color': 'rgb(' + String(translateSwitchToColor(outcome)) + ', 0, 0'})
      }
      if (outcome < 0) {
        $('#' + left_right[horizontal] + 'Deck' + jsUcfirst(top_bottom[vertical])).css({'color': 'rgb(0, ' + String(translateSwitchToColor(outcome)) + ', 0'}) 
      }
      if (outcome == 0) {
        $('#' + left_right[horizontal] + 'Deck' + jsUcfirst(top_bottom[vertical])).css({'color': 'rgb(0, 0, 0'})      
      }
    }
  }
